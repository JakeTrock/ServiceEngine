...
import { defaultConnect } from "../data/config";
...
const SvcPage = ({ match, location, history }) => {
    ...
    const reportUtil = () => defaultConnect
        ...
        .then(() => toast("Successfully Reported Utility"))
        .catch((e) => toast(e));

    //like a util if logged in
    const likeUtil = () => defaultConnect
        .post(("/utils/like/" + currentComponent.serviceUUID), {}, {
            headers: {
                Authorization: userToken
            }
        })
        .then(() => toast("Successfully Liked Utility"))
        .catch((e) => toast(e));

    //dislike a util if logged in
    const dislikeUtil = () => defaultConnect
        .post(("/utils/dislike/" + currentComponent.serviceUUID), {}, {
            headers: {
                Authorization: userToken
            }
        })
        .then(() => toast("Successfully Disiked Utility"))
        .catch((e) => toast(e));
    ...
    return (
        <div id="helper">
            ...
            {userToken &&
                <div>
                    {/* TODO:show likes and dislikes in parens */}
                    <button onClick={() => likeUtil()}>Like()</button>
                    <button onClick={() => dislikeUtil()}>Dislike()</button>
                    <h6>Report utility</h6>
                    <Collapsible>
                        <textarea ref={reportBox}></textarea>
