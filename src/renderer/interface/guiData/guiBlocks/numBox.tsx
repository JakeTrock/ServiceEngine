...
function NumBox(props) {
  const { visible, disabled, size, value, min, max, step, required } =
    props.objProps;
  ...
  const validate = (e) => {
    ...
    if (badVal) {
      ...
      toast(
        `The value of this numberbox is limited to between ${
          min || 0
        } and ${max} characters`
      );
    } else if (catchall) {
      hookset.current.value = min;
      toast(
        'Misc. validation error on numberbox. Try using the arrows on the side.'
      );
    }
  };
  ...
  return (
    <input
      ...
      step={step}
      ref={hookset}
      style={{ visibility: helpers.toggleVis(visible), fontSize: size }}
      min={min}
      max={max}
      defaultValue={value}
    />
