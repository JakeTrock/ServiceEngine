...
const validDate =
  /^(([0-9]{4})((-[0-9]{2}){2}(( |T)?([0-9]{2}:){2}[0-9]{2}((\+[0-9]{2}(:[0-9]{2})?)|Z)?)?|(-W[0-9]{2}(-[1-7])?)|(-[1-3]?[0-9]{2})))$/;

function DateBox(props) {
  const { visible, disabled, size, value, min, max, required } = props.objProps;
  const hookset = React.useRef(null);
  if (validDate.test(value)) {
    toast('Improperly formatted value!');
  }
  if (validDate.test(min)) {
    toast('Improperly formatted min!');
  }
  if (validDate.test(max)) {
    toast('Improperly formatted max!');
  }
  const validate = (e) => {
    ...
    if (badVal) {
      ...
      toast(
        `The value of this datebox is limited to between date ${
          min || 0
        } and ${max}`
      );
    } else if (catchall) {
      hookset.current.value = min;
      toast(
        'Misc. validation error on datebox. Try using the calendar picker.'
      );
    }
  };
