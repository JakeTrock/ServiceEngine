...
function ListBuilder(props) {
  ...
  const { maxListLength, minListLength } = props.validate || {};

  const id = props.uuid;

  const [allComps, setAllComps] = React.useState<string[]>(childNodesCurrent);
  const [allVals, setAllVals] =
    React.useState<(string | number | boolean | object)[]>(value);
          ...
          <button
            ...
            onClick={(e) => {
              ...
              if (newVal !== '') {
                ...
                setAllVals([
                  ...allVals,
                  childNodesPossible[newVal].defaults.value,
                ]);
  ...
  const minusButton = (i) => (
    <button
      ...
      onClick={() => {
        ...
        setAllVals(
          allVals.slice(0, i).concat(allVals.slice(i + 1, allVals.length))
        );
        setAllComps(
          JSON.parse(
            JSON.stringify(
              allComps
                .slice(0, i)
                .concat(allComps.slice(i + 1, allComps.length))
            )
          )
        );
      }}
