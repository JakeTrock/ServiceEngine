...
function Container(props) {
  const { visible, disabled, collapsible, width, height, childNodes, label } =
    props.objProps;
  const id = props.uuid;
      ...
      Object.entries(ohooks).forEach(([key, func]) => {
        switch (key) {
          ...
          case 'load':
            ...
            break;
          case 'keyPressed':
            hookset.current.addEventListener('keypress', (e) => {
              e.preventDefault();
              return (func as Function)({
                visible: active,
                value: e.keyCode,
  ...
  return (
    <fieldset
      ...
      style={{
        ...
        width,
        height: active ? height : '0em',
      }}
    >
      <legend>
        {collapsible && (
          <h5 style={{ display: 'inline' }} onClick={() => setActive(!active)}>
            {active ? '⯆' : '⯈'}
          </h5>
        )}
        {label}
