...
function TextBox(props) {
  ...
  const validateTxt = (e) => {
    ...
    if (maxChars && minChars && minChars > maxChars)
      return toast('invalid max/min values');
    if (
      (maxChars && val.length > maxChars) ||
      (minChars && val.length < minChars)
    ) {
      // bad length
      hookset.current.value = hookset.current.value.substring(
        0,
        maxChars - 1 + (minChars || 0)
      );
      toast(
        `The length of this textbox is limited to between ${
          minChars || 0
        } and ${maxChars} characters`
      );
    } else if (validateRegex && validateMessage && val.match(validateRegex)) {
      // regex violation
      ...
      (useBlacklist
        ...
        : useWhitelist
        ? wordList.find((w) => val.indexOf(w) < 0)
        : true)
    ) {
      // wordlist violation
      hookset.current.value = '';
      ...
      Object.entries(ohooks).forEach(([key, func]) => {
        switch (key) {
          ...
          case 'mouseOut':
            ...
            break;
          case 'load':
            (func as Function)({ value: hookset.current.value });
            break;
          case 'keyPressed':
            hookset.current.addEventListener('click', (e) => {
              e.preventDefault();
