...
function GuiEditPanel(props) {
  const [ifOrder, setifOrder] = React.useState<IFaceBlock[]>(
    props.initValues && props.initValues.length > 0 ? props.initValues : []
  );
  ...
  React.useEffect(() => {
    mcedit.current!.addEventListener(
      'contextmenu',
      function (event) {
  ...
  const updTxt = () => {
    try {
      ...
      if (inst.match(/,(?=\s*[\)\}\]])/))
        return toast('you must remove all trailing commas!');
  ...
  const insText = (txt) => {
    if (
      mcedit.current!.selectionStart ||
      mcedit.current!.selectionStart == '0'
    ) {
      const startPos = mcedit.current!.selectionStart;
      const endPos = mcedit.current!.selectionEnd;
      mcedit.current!.value =
        mcedit.current!.value.substring(0, startPos) +
        txt +
  ...
  return (
    <div onClick={exitclk} style={{ height: '100%' }}>
      <p>hint: right click to insert templates</p>
      <br />
      <button className="smbutton" onClick={updTxt}>
        save
      </button>
      <br />
      <textarea
        defaultValue={JSON.stringify(ifOrder, null, 2) || '[\n\n]'}
        style={{ height: '60vh', width: '40vh', fontFamily: 'tahoma' }}
        ref={mcedit}
      />
