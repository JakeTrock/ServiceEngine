...
const getAllLibNames = () => {
  ...
  return ['ffmpeg', 'fileUtils', 'magick', 'threedmc'];
};
...
const Editor = (props) => {
  ...
  const reloadRes = () => {
    // TODO: optimal insertion location for validation
    setResultData({ ...initVals, scheme: ifSchema });
    // TODO: add in part to add res script
  };

  const assembleScript = () => {
    return `const init=(imports)=>{
              ${initVals.binariesUsed.map(
                (name: string) => `const ${name} = imports.libraries.${name};\n`
              )}\n
              ${varblocks}\n
              return {
              ${findEvts(props.blocks)
                .map(
                  (name: string, i) =>
                    `{${name}}: (event, formAccess, additional, notify) => {${scripts[i]}\n}, \n\`}`
                )
                .join('')}
            }}; (() => { window.glue = init; })(); }`;
  };
  ...
  const propform: IFaceBlock[] = [
    {
      id: 'label',
      defaults: { visible: true, size: '1em', label: 'Title:' },
      uuid: 'fd117200-5bd0-4f4f-aae6-5c475f050fb7',
    },
    {
      id: 'textbox',
      defaults: {
      ...
      hooks: { change: { name: 'set', additional: { name: 'title' } } },
      uuid: '5dab4339-5b5f-4d98-8c65-8daa8cd37391',
    },
    {
      id: 'label',
      defaults: { visible: true, size: '1em', label: 'Libraries:' },
      uuid: '1f9ad96a-4453-43e6-aff4-d9f0af0be38d',
    },
    {
      id: 'listbuild',
      defaults: {
        ...
        size: '1em',
        width: '20em',
        value: initVals.binariesUsed,
        childNodesCurrent: initVals.tags.map((x) => 'onechoice'),
        childNodesPossible: {
          onechoice: {
            id: 'onechoice',
            defaults: {
      ...
      hooks: { change: { name: 'set', additional: { name: 'binariesUsed' } } },
      uuid: '8df0af40-c88a-4af0-a51e-e354df6ad06e',
    },
    {
      id: 'label',
      defaults: { visible: true, size: '1em', label: 'Tags:' },
      uuid: '11dd756a-4453-43e6-aff4-c4dccf0be38d',
    },
    {
      id: 'listbuild',
      defaults: {
        ...
        width: '20em',
        value: initVals.tags,
        childNodesCurrent: initVals.tags.map((x) => 'textbox'),
        childNodesPossible: {
          textbox: {
            id: 'textbox',
            defaults: {
      ...
      hooks: { change: { name: 'set', additional: { name: 'tags' } } },
      uuid: '8ddf0ef0-c88a-4af0-a51e-eacf1c05c06e',
    },
    {
      id: 'label',
      defaults: { visible: true, size: '1em', label: 'Description:' },
      uuid: '75fb57df-7379-4d32-a5a3-d030db8d88ef',
    },
    {
      id: 'textbox',
      defaults: {
        ...
        size: '1em',
        multirow: true,
        value: initVals.description,
      },
      hooks: { change: { name: 'set', additional: { name: 'description' } } },
      uuid: 'a77429da-9fec-4457-9e3f-725a84b081d4',
    },
  ];
    ...
    <>
      <div className="left-0 w-1/2 border-black border-solid border h-full overflow-scroll overflow-x-hidden pt-5 fixed">
        <TabsContainer>
              ...
              exports={{
                set: (e, formAccess, additional, mkdialog) => {
                  if (additional!.name === 'binariesUsed') {
                    if (e!.value.length > initVals.binariesUsed.length) {
                      // greater
                      // add new value
                      const seekVal = e!.value.find(
                        (v) => !initVals.binariesUsed.includes(v)
                      );
                      if (seekVal) {
                        const temp = initVals;
                    ...
                    } else if (e!.value.length < initVals.binariesUsed.length) {
                      // less
                      const seekVal = initVals.binariesUsed.find(
                        (v) => !e!.value.includes(v)
                      );
                      const temp = initVals;
          ...
          <Tab label="Program">
            <ProgEditPanel
              imports={initVals.binariesUsed}
              blocks={ifSchema}
              initVars={varblocks}
              varSet={setVarblocks}
              initScripts={scripts}
              scriptSet={setScripts}
            />
          </Tab>
          <Tab label="Interface">
        ...
        {resultData ? (
          <div id="helper">
            ...
            >
              <GuiRender
                scheme={ifSchema}
                setScheme={setifSchema}
                exports={exports}
              />
            </div>
          </div>
        ) : (
          <h1>no data yet, hit "Run"</h1>
        )}
      </div>
    </>
  );
