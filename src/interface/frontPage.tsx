...
import { login, logout } from "./data/helpers";
import { getTrending } from "../graphql/queries";
import { ModelSortDirection } from "../API";
import { usrCreds } from "./data/interfaces";
// markup
const IndexPage = ({ match, location, history }) => {
  ...
  const [frontpage, setFrontPage] = React.useState<Object>();
  //logs out the current user, reloads page

  const fetchFrontPage = async (): Promise<any> => {//TODO:typeme
    return await API.graphql(graphqlOperation(getTrending, {
      $numLikes: { gt: 10 },
      $createdAt: { lt: new Date() },//TODO:would this work on a string?
      $sortDirection: ModelSortDirection.ASC,//TODO:could be desc
      $filter: {
  ...
  React.useEffect(() => {
    ...
    fetchFrontPage()
      .then((fpdata) => setFrontPage(fpdata))
      .catch((e) => toast(e));
  }, []);
  ...
  return (
    <div id="holder">
      ...
      <div>
        ...
        />
        <input ref={searchBox} /><button onClick={() => window.open(`https://www.google.com/search?q=site%3Asengine.cc+${encodeURIComponent(searchBox.current.replaceAll("%20", "+"))}`, '_blank')}>search</button>
        {userToken.confirmed ?
          <div>
            <h5>{userToken.username}</h5>
            <button onClick={() => history.push('/dash')}>Dashboard</button>
            <button onClick={() => logout()}>Log out</button>
          </div>
          :
          <button onClick={() => history.push('/auth')}>login</button>}
        {/* TODO:put suggested frontpage apps here */}
        {/* TODO:add tagsort */}
      </div>
    </div>
  );
