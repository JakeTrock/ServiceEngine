...
export default function GuiRender(props) {
    ...
    const formAccess = (action: "get" | "set" | "add" | "del", key: string | number, kvpset) => {
        console.log(action, key, kvpset)
        if (action && currentInterface.length) {
            switch (action) {
                case "del":
                    if (key) { //delete form element, provide a uuid in the key slot and any match will be deleted
                ...
                case "set":
                    if (key && kvpset) {//set value of a form element, provide a uuid in the key slot and the component that matches it will have any keys which are specified changed to new values
